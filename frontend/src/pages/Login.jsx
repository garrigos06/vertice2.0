...
export default function Login() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      else if (u.role === "ADMIN" || u.role === "SUPER_ADMIN") navigate("/admin");
      else navigate("/conta");
    } catch (err) {
  ...
  return (
    <PublicLayout>
      <section className="max-w-md mx-auto px-5 py-16">
        ...
        <p className="text-white/60 mb-8">Acesse sua conta Vértice Sports.</p>
        ...
        <div className="mt-6 flex justify-between text-sm">
          <Link to="/recuperar-senha" data-testid="login-forgot-link" className="text-white/60 hover:text-white">
            Esqueci minha senha
          </Link>
          <Link to="/cadastro" data-testid="login-register-link" className="text-[#CCFF00] hover:text-[#e6ff4d]">
            Criar conta
          </Link>
        </div>
      </section>
