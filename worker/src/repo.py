...
def _bet_row_to_dict(row: dict) -> dict:
    return {
        ...
        # Define se este FREE pode ser mostrado
        # completamente sem autenticação.
        "is_public_preview": bool(
            row.get("is_public_preview", 0)
        ),
...
async def bets_insert(
    ...
    await d1_run(
        ...
        bet["risk"],
        bet["rationale"],
        bet.get("scheduled_at"),
        bet["required_plan"],
        bet.get("external_url"),
        1 if bet["featured"] else 0,
        1 if bet.get(
            "is_public_preview"
        ) else 0,
        bet.get("image_url"),
        bet["status"],
        1 if bet["published"] else 0,
        bet["created_at"],
        bet["updated_at"],
    )
...
async def bets_update(
    db,
    bet_id: str,
    updates: dict,
) -> Optional[dict]:
    if not updates:
        return await bets_get(
            db,
            bet_id,
        )
...
async def bets_set_public_preview(
    db,
    bet_id: str,
    enabled: bool,
) -> Optional[dict]:
    """
    Ativa ou desativa a amostra pública de um bilhete.

    Ao ativar:
    - remove a marca de qualquer outra amostra;
    - ativa o bilhete solicitado.

    A validação de que somente FREE pode ser público
    também existe no banco através de trigger.
    """
...
async def prt_get_by_hash_unused(
    db,
    token_hash: str,
) -> Optional[dict]:
    return await d1_first(
